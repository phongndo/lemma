...
| `pty_writer_test.cpp` | KEEP | All seven cases. They protect partial progress, terminal-response-before-input ordering, EINTR attempt bounds, per-pane/global fairness budgets, hard-error suffix retention, reusable bounded storage, and rejection of impossible writer progress. |
...
| `input_router_test.cpp` | KEEP | All 13 cases. Compile-time map rejection protects unique/resolved/bounded contexts. Router cases protect deferred-prefix replay, escape-sequence batching, one-shot/transient lifetime, held-key ownership, modifier normalization, and direct bindings. These prevent leaked prefixes, replayed input, and commands firing in the wrong Attachment context. |
...
| `host_terminal_theme_test.cpp` | KEEP | All four cases. They protect fragmented OSC replies, exact passthrough of non-theme bytes, bounded storage reuse, and the complete bounded query surface. |
...
| File | Classification | Tests and bug caught |
| --- | --- | --- |
| `terminal_test.cpp` | KEEP | `RejectsInvalidAndUnfundedConfigurations`; damage cases; caller-owned screen formatting; changed-row/span/scroll rendering; grapheme convergence; resize mapping; key, application-cursor, focus, mouse, alternate-scroll, and paste mode consequences; graphics disable policy; sticky response overflow; truthful identity/geometry responses; typed effects; tracked selection/copy movement/wide-cell/normalization/viewport/search/checkpoint/reflow/highlight contracts; and allocator accounting. Each exercises a Lemma-owned assumption through only Lemma public types. |
...
| `render_composition_test.cpp` | KEEP | All 23 cases. They protect atomic multi-pane placement, bounded status/edit/copy overlays, status-row geometry, overflow visibility, incremental status/pane damage, declared separators and junctions, focused outer-mode ownership, mouse-capture projection, suspended layout clearing, non-overlap/focus validation, and output bounds. Exact byte assertions are retained only where ANSI presentation is Lemma's contract; mode tests also parse the composed consequence. |
