...
A five-repetition release `interactive-output` smoke after adding the procedure/control surface, held-pane exit policy, and signal-driven child reaping measured key-to-PTY p50/p95 of 0.065/0.091 ms and key-to-visible p50/p95 of 0.143/0.161 ms. This small dirty-tree diagnostic is not a replacement baseline, but it found no interaction regression against the retained larger distributions below; daemon wakeups remained 20 for the completed sample. The exact reproduction command was `./bench mux`.

Two same-host thirty-sample release runs on August 18, 2026 qualified moving prefix interpretation into a compiled daemon input-policy router. Taking the larger after-change distribution, key-to-PTY p50/p95 was 0.106/0.225 ms versus the retained 0.101/0.187 ms comparison baseline below; key-to-visible was 0.528/0.893 ms versus 0.759/1.119 ms, with the same 633-byte median client output. The repeat measured 0.098/0.120 ms key-to-PTY and 0.482/0.628 ms key-to-visible, demonstrating meaningful host variation; the 0.038-ms key-to-PTY p95 increase in the larger run remains reported rather than discarded. The focused router microbenchmark, including the runtime rollback-checkpoint query, measured 5.12 ns for an unbound four-byte run and 8.98 ns for a legacy command in a persistent context. A structured-key forward press/release cycle measured 9.05 ns total, and a captured structured-key context repeat measured 9.93 ns. Three post-review thirty-sample runs measured key-to-PTY p50/p95 ranges of 0.121–0.132/0.197–0.210 ms and key-to-visible ranges of 0.689–0.803/1.133–1.347 ms, again showing host variance and retaining the slower distributions rather than hiding them. The allocation audit, extended through routing, remained at zero general and terminal-quota allocations. Reports are `input-router-after-interactive-output-30{,-repeat}.json` and `input-router-review-interactive-output-30{,-repeat,-third}.json`.
...
A three-repetition release microbenchmark on August 17, 2026 measured the allocation-free fixed-point layout paths. Median CPU was 59.0/84.0/183/590 ns for 1/4/16/64-pane balanced projection. Copying and keyboard-resizing a 64-pane candidate layout, including before/after projection and invariant-preserving ratio update, measured 1.542 us median. A post-topology-change five-repetition run measured 1.475 us for the same resize candidate and 321 ns for a maximum-layout copy plus leaf-ID swap. The swap measurement includes structural validation but not viewport projection or Runtime convergence. Exact projected-divider hit-testing at mouse press measured 618 ns. A later five-repetition measurement of one captured-divider candidate resize plus exact divider-rectangle projection measured 2.108 us median. Each distinct mouse-cell position performs that bounded candidate calculation and commits layout, PTY, and Ghostty geometry through one transaction; there is no deferred PTY endpoint. A separate five-repetition maximum-depth 64-pane layout measured 1.153 us projection and 1.226 us to hit its deepest divider, bounding the deliberately adversarial tree shape rather than only the balanced case. This is semantic layout work only; it excludes Ghostty reflow, PTY `TIOCSWINSZ`, frame composition, and client transport, so it establishes bounded Core cost rather than end-to-end live-resize latency.

```sh
./build/release/lemma_benchmarks \
  --benchmark_filter='^benchmark_layout_(projection|resize_candidate|swap_candidate|divider_hit|divider_resize_candidate)' \
  --benchmark_min_time=0.05s --benchmark_repetitions=3 \
  --benchmark_report_aggregates_only=true
```

After making live resize PTY-first and atomic, a five-repetition live-divider microbenchmark alternated one root separator cell and included candidate layout publication, every changed Ghostty terminal resize, and complete pane composition. Median CPU was 466.415/482.545/514.083/589.970 us for 2/4/16/64 panes, with unchanged average frames of 21,196/20,941/22,216/24,586 bytes (`live-divider-geometry-sync.json`). The retained pre-change medians were 482.899/500.827/536.456/631.575 us. This benchmark excludes PTY ioctls, child scheduling, socket transport, and the outer terminal, so it shows no regression in the unchanged reflow/composition work rather than claiming pointer-to-screen latency.

A separate seven-repetition benchmark alternated `TIOCSWINSZ` against a real PTY whose foreground child handled `SIGWINCH`. Median synchronous cost was 13.613 us wall and 11.172 us parent CPU per changed pane (`live-divider-pty-resize.json`). A common two-pane divider update adds two such calls: conservatively combining the independently measured medians gives about 493.6 us wall and 488.8 us parent CPU, versus the retained 482.9-us pre-change daemon CPU median (about +1.2% CPU). If all 64 panes change, a deliberately conservative linear projection is 1.46 ms wall/1.31 ms parent CPU including reflow and composition; it is not an integrated latency measurement and excludes application redraw work after `SIGWINCH`. The one-geometry-message-per-session reactor budget remains the fairness bound.

Reproduce with:

```sh
./build/release/lemma_benchmarks \
  --benchmark_filter='^benchmark_live_divider_(resize/|pty_resize$)' \
