...
| Copy/search state | inline in `Session` | included above; 256-byte maximum query plus cursor/status fields | attach copy-mode entry / detach reset | bounded incremental search retains no terminal text grid or match list | none |
| Pending user copy | one nullable session-owned byte array | 0 normally; at most 1 MiB, additionally limited by the attached frame capacity | `Enter`/`y` in copy mode / clipboard frame queue or detach | allocation/format failure leaves copy mode active with visible status; successful bytes are base64-encoded into the retained frame and released | explicit user copy only |
...
| Terminal adapter | `Terminal::Impl` RAII owner | prior census plus six nullable selection-gesture handles | pane create / close | `make_unique` failure rejects pane; gesture objects remain lazy until first pointer gesture | create only; first pointer gesture may allocate |
...
| Scrollback page list | Ghostty terminal owner | active-screen minimum; 10,000-B default byte quota; 1,000,000-B byte and optional line hard limits | grows/prunes on PTY output / pane close | page-granular and lazy; independent byte/line limits prune on the first reached dimension; idle compression is bounded and representation-only | quota-owned PTY history exception |
