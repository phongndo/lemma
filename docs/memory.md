...
| Attachment | Core-owned one-per-session value | 616 B | contains stable IDs, bounded committed/draft copy-search queries, explicit copy viewport and phase policy, and generation-safe pane or divider mouse capture identity; ordinary wheel scrolling stays in Ghostty; no independent allocation or duplicate divider coordinate |
| AttachmentRuntime | Runtime-owned one-per-session value | 9,064 B | owns connection, decoder, frame/output progress, runtime copy continuation, one live-divider PTY checkpoint/deadline, bounded pending client-work state, backpressure, and teardown; no hot-path allocation or lookup |
| Runtime session record | one stable aggregate allocation per live session | 80,456 B including Session, Attachment, AttachmentRuntime, terminal theme, and connection generation | preserves direct addresses and replaces the former 94,488-B mixed Session; the unused 14-KiB command trace was removed |
| Tab | one owner per live tab | 3,600 B | includes a 2,540-B fixed-capacity split tree whose branch ratios fit without increasing the prior Tab size; allocated on create and candidate resize remains bounded stack storage |
