...
| Runtime session record | one stable aggregate allocation per live session | 81,712 B including Session, Attachment, AttachmentRuntime, terminal theme, and connection generation | preserves direct addresses; the Session-scoped pane index adds bounded identity storage while removing pane slots from every Tab |
...
| Semantic Pane | Core-owned per live pane | 24 B | contains Session-scoped generational identity, owning TabId, and committed geometry; staged creation publishes it only with a prepared runtime counterpart |
