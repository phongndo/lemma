...
| Pane launch argv | optional semantic Pane owner | 0 for the login-shell default; exact retained payload up to 65,535 B for an explicit initial command | allocated only during staged explicit launch and released with the pane |
...
| Pending connection | one lazy live setup | about 144 KiB base; validated launch setup may additionally retain exact environment and argv payloads up to 65,535 B each; 128 slots | payload allocation is cold, bounded, and released with the setup; allocation failure rejects only that connection |
