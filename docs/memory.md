...
| AttachmentRuntime | Runtime-owned one-per-session value | 9,040 B | owns connection, decoder, frame/output progress, runtime copy continuation, bounded pending client-work state, backpressure, and teardown; live divider resize retains no runtime geometry or deadline |
| Runtime session record | one stable aggregate allocation per live session | 81,688 B including Session, Attachment, AttachmentRuntime, terminal theme, and connection generation | preserves direct addresses; the Session-scoped pane index adds bounded identity storage while removing pane slots from every Tab |
