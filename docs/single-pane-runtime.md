...
`single_pane.cpp`. Spaces now support bounded windows and split panes while retaining the
...
command but no space runtime logic. `apps/lemma/main.cpp` only delegates to `lemma::app::run`.
...
Space creation, lookup, listing, and removal are handled by the authoritative engine.

### Core — `src/core/`

Owns up to 64 running spaces, 1,024 windows, and 4,096 panes in one reactor. Each space is
...
tree and focus/zoom state. The space owns its ordered window slots, active-window selection,
attached daemon-side client descriptor, protocol-message state, frame scheduling, and backpressure
state. The reactor borrows the daemon listener and remains the sole owner of mutable space and
terminal state.
...
- up to 64 validated named spaces in one per-user daemon;
- up to 16 windows and 64 shells, PTYs, and canonical Ghostty terminals per space;
- generational window IDs that reject stale slot references;
- one attached client plus independent space/window-list and kill control connections;
...
- spaces and panes do not yet use separate generational stores; windows use generational IDs
...
- new spaces inherit the daemon's original environment and working directory;
...
- windows cannot be linked across spaces;
...
Existing attached-client pane/window actions, detach, and CLI space stops already pass through
