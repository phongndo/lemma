...
| Machine-readable semantic API | Working | The owner-only per-user Unix endpoint accepts persistent lock-step `lemma.action/v1` Action RPC and `lemma.proc/v1` Proc RPC. Both use one daemon-owned Action executor and canonical nested results. Proc validates up to 64 ordered Actions and typed backward references before side effects. `lemma action`, `lemma proc`, and `lemma events` are the shell and coding-agent frontends; dedicated integrations may use CONTROL and OBSERVE directly. `api schema --json` prints the embedded JSON Schema 2020-12 contract offline. |
...
| Pane automation | Working | `lemma action pane list/send/capture` targets stable IDs and returns canonical Action results. Pane listings include zero-based content-grid coordinates and dimensions, preserving observable spatial order across swaps. `lemma events` exposes filtered initial snapshots plus coalesced semantic, process, and optional plain-text screen updates as NDJSON. Waiting and streaming are observation concerns rather than Proc Actions. Semantic key/paste input and historical capture/search are not yet exposed. |
