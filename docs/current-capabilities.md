...
| Agent skill | Working | `skill` prints a version-matched single-file Agent Skills guide without contacting the daemon. It directs agents to live introspection, focus-preserving creation, atomic semantic input, finite waits, and bounded capture. |
| Help/version/errors | Working | Dedicated output and nonzero invalid-command behavior exist. Every structured Action has offline `lemma action DOMAIN OP --help` grammar; Proc and Events expose focused help as well. |
...
| Machine-readable semantic API | Working | The owner-only per-user Unix endpoint accepts persistent lock-step `lemma.action/v1` Action RPC and `lemma.proc/v1` Proc RPC. Both use one daemon-owned Action executor and canonical nested results. Proc validates up to 64 ordered Actions and typed backward references before side effects. `lemma action`, `lemma proc`, and `lemma events` are the shell and coding-agent frontends; dedicated integrations may use CONTROL and OBSERVE directly. Session revisions support conditional queries/mutations, and terminal generations establish capture/input/wait freshness. `api schema --json` prints the embedded JSON Schema 2020-12 contract offline. |
...
| Capability | Status | Current behavior |
| --- | --- | --- |
| Tabs | Working | Up to 16 tabs per session; create with exact argv/cwd/exit policy and explicit `created`/`preserve` focus policy, cycle, numeric or CLI select, list, inspect, close, stable absolute CLI placement or relative key reorder (`C-b P`/`C-b N`), and bounded title override. Detailed inspection returns the canonical split tree, ratios, focus, zoom, suspension, title, and geometry. Display order is one Session-owned permutation independent of stable IDs and storage slots. |
| Split panes | Working | Nested left/right and top/bottom binary splits, up to 64 panes per session and 4,096 daemon-wide. `lemma action pane split` targets a stable pane ID and accepts exact argv, explicit cwd, `--hold`, and explicit `created`/`preserve` focus policy. New tab/split processes use the Session's captured environment and creation cwd unless an explicit cwd is supplied. |
...
| Pane automation | Working | `lemma action pane list/inspect/input/capture/wait` targets stable IDs and returns canonical Action results. `pane.input` atomically admits up to 64 ordered text, opaque-paste, and logical-key events through Ghostty's canonical modes; legacy `pane.send` remains wire-compatible. Capture has passive `visible`, bounded `recent`, and OSC-133-derived `last-command` sources, plain/ANSI format, rendered/logical wrapping, terminal generation, and explicit truncation. `pane.wait` defaults to bounded child-process completion, infers the current Pane in CLI context, supports literal-output and semantic-prompt conditions, and composes inside Proc without blocking reactor progress. `lemma events` accepts up to eight Pane filters and emits initial snapshots followed by process, compact terminal-invalidation, optional screen, closure, and semantic revision updates. |
...
| Pane identification UI | Partial | IDs appear in listings, creation results, procedure results, typed command targets, and detailed Pane inspection, but there is no pane overlay or naming UX. |
...
| Scrollback and reflow | Working | Ghostty owns canonical history, viewport, reflow, and incremental cold-page compression. The default and per-pane hard byte bound match the pinned Ghostty 50,000,000-byte surface default, optional line bounds remain independent, and Runtime admits configured pane capacity against a 3.2-GB daemon aggregate reservation bound. Public recent capture formats a bounded caller-selected tail directly from canonical history without moving the viewport or installing a selection. |
| Selection/search/formatting | Working | Adapter wraps tracked selection, viewport, bounded search, visible/recent formatting, OSC-133 command-output selection, compact terminal inspection, and incremental compression primitives. Child-reported title/PWD and prompt state retain explicit provenance at the public boundary. |
