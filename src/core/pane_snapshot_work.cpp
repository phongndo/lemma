...
PaneSnapshotWork::PaneSnapshotWork(vt::Terminal&& terminal) noexcept
    : state_(std::in_place_type<Active>, std::move(terminal)) {}

PaneSnapshotWork::~PaneSnapshotWork() = default;

// std::variant reports potentially throwing access even though every branch is tag-checked.
// NOLINTNEXTLINE(bugprone-exception-escape)
auto PaneSnapshotWork::phase() const noexcept -> PaneResidencyPhase {
...
auto PaneSnapshotWork::active_terminal() noexcept -> vt::Terminal* {
  auto* const active = std::get_if<Active>(&state_);
  return active == nullptr ? nullptr : &active->terminal;
}

auto PaneSnapshotWork::active_terminal() const noexcept -> const vt::Terminal* {
  const auto* const active = std::get_if<Active>(&state_);
  return active == nullptr ? nullptr : &active->terminal;
}

// NOLINTNEXTLINE(bugprone-exception-escape)
auto PaneSnapshotWork::snapshot_bytes() const noexcept -> std::size_t {
...
// NOLINTNEXTLINE(bugprone-exception-escape)
auto PaneSnapshotWork::begin_parking(const vt::TerminalOptions& restore_options,
...
// NOLINTNEXTLINE(bugprone-exception-escape)
auto PaneSnapshotWork::finish_parking(const SnapshotTestCorruption corruption) noexcept
  ...
  cold.state.emplace<Parked>(std::move(snapshot), options);
  return {};
}

// NOLINTNEXTLINE(bugprone-exception-escape)
void PaneSnapshotWork::cancel_parking() noexcept {
  auto* const cold_owner = std::get_if<std::unique_ptr<ColdResidency>>(&state_);
  if (cold_owner == nullptr) {
    return;
  }
...
// NOLINTNEXTLINE(bugprone-exception-escape)
auto PaneSnapshotWork::begin_unparking() noexcept -> std::expected<void, vt::Error> {
  auto* const cold_owner = std::get_if<std::unique_ptr<ColdResidency>>(&state_);
  if (cold_owner == nullptr) {
    return std::unexpected(vt::Error::invalid_state);
  }
  ...
  cold.state.emplace<Unparking>(std::move(snapshot), options, std::move(*payload),
                                std::move(decoder));
  return {};
}

// NOLINTNEXTLINE(bugprone-exception-escape)
auto PaneSnapshotWork::restore_one_history_page() noexcept -> std::expected<bool, vt::Error> {
  auto* const cold_owner = std::get_if<std::unique_ptr<ColdResidency>>(&state_);
  if (cold_owner == nullptr) {
    return std::unexpected(vt::Error::invalid_state);
  }
...
// NOLINTNEXTLINE(bugprone-exception-escape)
void PaneSnapshotWork::cancel_unparking() noexcept {
  auto* const cold_owner = std::get_if<std::unique_ptr<ColdResidency>>(&state_);
  if (cold_owner == nullptr) {
    return;
  }
